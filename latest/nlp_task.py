...
flags.DEFINE_string("save_path", 'models/ptb/DRUM_2000_0.5_50_64/',
    ...
    def __init__(self, is_training, config, input_):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(num_steps):
                ...
                out, state = DRUM_cell(inputs[:, time_step, :], state)
                outputs.append(out)

        ##
        print('graph generated')
        output = tf.reshape(tf.concat(axis=1, values=outputs), [-1, D_size])

        # Output layer and cross entropy loss

        out_init = aux.orthogonal_initializer(1.0)
        softmax_w = tf.get_variable(
            "softmax_w", [D_size, vocab_size], initializer=out_init, dtype=tf.float32)
