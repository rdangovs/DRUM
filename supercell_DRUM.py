...
class HyperDRUMCell(tf.contrib.rnn.RNNCell):
  ...
  def __init__(self, num_units, forget_bias=1.0,
    use_recurrent_dropout=False, dropout_keep_prob=0.90, use_layer_norm=True,
    hyper_num_units=128, hyper_embedding_size=16,
    hyper_use_recurrent_dropout=False, normalization=0.5, eps=1e-12):
    '''Initialize the Layer Norm HyperLSTM cell.
    Args:
      num_units: int, The number of units in the LSTM cell.
      forget_bias: float, The bias added to forget gates (default 1.0).
  ...
  def __call__(self, x, state, timestep = 0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      x_mixed = _linear(x, self.num_units, True, kernel_initializer = w_init)
      
      state_new = rotate(x_mixed, r, h)
      cz = relu(x_mixed + state_new)

      if self.use_recurrent_dropout:
        cz = tf.nn.dropout(cz, self.dropout_keep_prob)
      new_h = u * h + (1 - u) * cz
      if self.normalization != None: 
        new_h = tf.nn.l2_normalize(new_h, 1, epsilon = self.eps) * self.normalization 
